<?php
	...
	$query = "SELECT `st_email_id`, `st_username`, `st_first_name`, `st_last_name`, `st_gender`, `dt_birthdate`, `im_profile_picture`, `st_category_name` FROM `registration` WHERE `in_id` = ".$_SESSION['usr_id'];
	...
	if(isset($_POST["sbm"])){
		...
		$n_txt_uname = $_POST["txt_uname"];
			...
			if(preg_match('/^[A-Za-z0-9]{6,}$/',$n_txt_uname)){
				...
				header("Location: http://localhost/forum/update_profile.php");
			}
				...
				if(in_array($file_ext, $allowed)){
					if($file_error === 0){
						if($file_size <= 1048576){
							...
							$query = "UPDATE `registration` SET `st_email_id`= '$n_txt_email',`st_username`= '$n_txt_uname',`st_first_name`= '$n_txt_fname',`st_last_name`= '$n_txt_lname',`st_gender`= '$n_rd_gen',`dt_birthdate`= '$n_dt_birth',`im_profile_picture`= '$file_name_new',`st_category_name`= '$cb_values' WHERE `in_id`=".$_SESSION["usr_id"];
								
								
								//echo "hello".$query;
								
								if(isset($_POST["txt_uname"])){
									$_SESSION["uname"] = $n_txt_uname;
								}
			...
			$query = "UPDATE `registration` SET `st_email_id`= '$n_txt_email',`st_username`= '$n_txt_uname',`st_first_name`= '$n_txt_fname',`st_last_name`= '$n_txt_lname',`st_gender`= '$n_rd_gen',`dt_birthdate`= '$n_dt_birth',`im_profile_picture`= '$pic_src',`st_category_name`= '$cb_values' WHERE `in_id`=".$_SESSION["usr_id"];
